...
class CategoryPicker extends Component {
  ...
  componentWillMount() {
	...
	grabCategories(specific) {
		axios.get('http://10.16.1.131:3000/categories', {params: {userID: this.props.userID}})
			.then((response) => {
  ...
  newCategory() {
    let category = this.state.newCategory;
		...
		if (category.length > 1) {
			axios.post('http://10.16.1.131:3000/categories', {category, color, userID: this.props.userID})
			.then(response => {
